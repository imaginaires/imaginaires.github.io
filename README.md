...
Ce dépôt contient trois déclinaisons esthétiques sous forme de fichiers HTML autonomes. Vous pouvez prévisualiser et choisir celle qui correspond le mieux à votre antenne :

1. **Dark Arcade / Playful Neon (`index.html`)** : Un thème sombre vibrant avec des accents cyan (#00ffff), violet et blanc, inspiré par l'univers des jeux vidéo rétro et le pixel art du logo.
2. **Modern Playful (`index-playful.html`)** : Un thème clair, doux et chaleureux, avec des cartes arrondies, des dégradés de couleurs fluides et une typographie accueillante.
3. **Minimalist Clean (`index-minimalist.html`)** : Une esthétique sobre, élégante et très structurée avec de grands espaces blancs, une typographie à empattements (serif) et un style éditorial haut de gamme.
...
1. **Fokez ou dupliquez** ce dépôt sur votre compte GitHub.
...
Pour adapter le site à votre propre association ou antenne :

### 1. Choisir le thème principal
Par défaut, le thème **Dark Arcade** est défini comme page d'accueil principale (`index.html`). 
Si vous préférez le thème **Modern Playful** ou **Minimalist Clean** :
- Supprimez ou renommez le fichier `index.html` actuel.
- Renommez le fichier de votre choix (`index-playful.html` ou `index-minimalist.html`) en `index.html`.

### 2. Mettre à jour les textes & liens
Ouvrez le fichier HTML dans un éditeur de texte (comme VS Code) et modifiez les sections suivantes :
...
### 3. Remplacer les logos et images
...
2. Double-cliquez sur `index.html` (ou l'un des autres fichiers) pour l'ouvrir directement dans votre navigateur web.
